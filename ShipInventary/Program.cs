...
of this panel is valid.";
        ...
        private List<string> EvaluateInvOwner(IMyTerminalBlock block)
        {
            ...
            if (sortedOres.Count > 0)
            {
                ...
                {
                    string key = oreKeys[i];
                    result.Add(formatDisplay(key, sortedOres[key].ToString(NumberFormat)));
                }
            ...
            if (sortedIngots.Count > 0)
            {
                ...
                {
                    string key = ingotKeys[i];
                    result.Add(formatDisplay(key, sortedIngots[key].ToString(NumberFormat)));
                }
            ...
            if (sortedComponents.Count > 0)
            {
                ...
                {
                    string key = componentKeys[i];
                    result.Add(formatDisplay(key, sortedComponents[key].ToString(NumberFormat)));
                }
            ...
            if (sortedAmmo.Count > 0)
            {
                ...
                {
                    string key = ammoKeys[i];
                    result.Add(formatDisplay(key, sortedAmmo[key].ToString(NumberFormat)));
                }
            }
        ...
        string formatDisplay(string key, string value)
        {
            return string.Format("[{1}] {0}", key, value.PadLeft(padding));
        }
